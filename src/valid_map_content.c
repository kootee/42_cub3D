...
int validate_and_set_player(t_cub *cub, int row, int col, char player_char, int *player_count)
{
    *player_count += 1;
    ...
    cub->player.ppos.x = col;
    cub->player.ppos.y = row;
...
int validate_player_position(t_cub *cub)
{
    ...
    for (i = 0; i < cub->map_height; i++)
    {
        for (j = 0; cub->map[i][j]; j++)
        {
            ...
            {
                if (validate_and_set_player(cub, i, j, cub->map[i][j], &player_count))
                    return (1);
            }
        }
    }
...
void calculate_map_width(t_cub *cub)
{
    ...
    while (cub->map[row])
    {
        size_t current_width = strlen(cub->map[row]);
...
int is_map_valid(t_cub *cub)
{
    ...
    while (cub->map[row])
    {
        ...
        while (cub->map[row][col])
        {
            ...
            if (count_players(cub->map, &player_count, row, col, cub))
                return (1);
